...
// Klientkomponent — bruker onClick for seogsa_click-event via dataLayer/GTM.
// Kan trygt brukes i server-komponenter (Next.js App Router: klient­grense).
...
interface SeOgsaProps {
  ...
  links: SeOgsaLink[]
  /** Opprinnelseskontekst for analytics-eventet (seogsa_click → from-prop).
   *  Settes av forelderen — f.eks. "kart", "sektor-yield", "naringsmegler".
   *  Utelatt → logger "ukjent". */
  from?: string
}

/**
 * SeOgsa — redaksjonell kryss­lenkblokk (klientkomponent).
 *
 ...
 *    ingen kort/skygger
 *  – Fyrer seogsa_click med { from, to } ved klikk (analytics 8A)
 */
export function SeOgsa({ heading, links, from }: SeOgsaProps) {
        ...
        {displayed.map(({ href, label, note }) => (
          <li key={href}>
            <Link
              ...
              onClick={() =>
                trackEvent("seogsa_click", { from: from ?? "ukjent", to: href })
              }
            >
              {label}
