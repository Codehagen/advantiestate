...
export const HelpPost = defineCollection({
  ...
  // Emit a lightweight slug→title index so CLIENT components (the MDX renderer's
  // <HelpArticles> link grid) can resolve help-article titles without importing
  // the full allHelpPosts corpus (every compiled MDX body) into the browser
  // bundle. That import bloated first-load JS on every MDX page and broke the
  // perf budget once the corpus grew. Change-guarded so it never rewrites on an
  // unchanged regen (which would loop the dev watcher).
  onSuccess: async (documents) => {
    const { readFileSync, writeFileSync } = await import("node:fs");
    const out = "src/lib/blog/help-link-index.ts";
    ...
    const body =
      ...
      "\n];\n";
    try {
      if (readFileSync(out, "utf8") === body) return;
    } catch {
