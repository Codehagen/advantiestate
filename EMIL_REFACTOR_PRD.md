...
- [x] **R6b — Leaflet marker a11y + 44px hit targets** (split from R6)
  > done 2026-06-29: NordNorge city markers got a `NordNorgeMarker` sub-component mirroring `CityMarker` (role=button + tabindex + aria-label + aria-pressed via the `add` handler, Enter/Space keydown). On BOTH maps the visible marker is now `interactive:false` and a transparent CircleMarker (`radius={Math.max(radius,22)}`, `fillOpacity 0`, `weight 0`) on top carries the ref/handlers/a11y, so the tap target clears 44px while the dot stays its data size. ⚠️ Leaflet changes — `QA1` must confirm both maps still render + select. build exit 0, lint 0 errors/0 warnings.
...
## Post-loop cleanup (done)

- [x] **Q1 — Low-severity audit §6 quick-wins** (12-agent parallel workflow, one per file)
  > done 2026-06-29: commit `a3805a7`. Tap targets→44px (Input toggle, CalculatorCTA, AnimatedCTA + press feedback, CSS `.hs-pill/.hs-chip/.hs-sortbtn`, `.ed-lb-close/.ed-lb-nav`); transition hygiene (Input `transition-all`→colors; `.switch` named; `.hot`/`.hs-row`/`.hs-mr` drop animated padding; `.rb-fill`→house curve; `.bench-*`→transform snap, rule 12); a11y (ListingsBrowser `role=group` ×3; HelpSearch/HelpLibrary fold-aware highlight for `Bodø`; LocationMdx typed props; PhotoBand optional alt); layout-shift/tokens (Select drop checked `font-semibold`; `.ed-lb-counter` tabular-nums; `.ei-alert` 16px iOS-zoom); taste (AnalyseportalShell city-toggle + keyboard tab nav un-animated; FeatureDivider literal delays). build 0, lint 0/0. **Follow-up:** `.bench-*` smooth-transform needs YieldCalculatorV2 to drive a `--bench-x` px var (it snaps for now — correct for keystroke-driven).
- [x] **Q2 — PropertyMapLeaflet mobile scroll-trap**
  > done 2026-06-29: commit `28d289f`. `dragging/touchZoom/doubleClickZoom={false}` on the fixed embeds so one-finger touch scrolls the page (property pages, city-panel, OfficeMap) instead of panning the map.

## Notes
- (Resolved) The §6 quick-wins + PropertyMapLeaflet scroll-trap are now done (Q1/Q2 above), applied via parallel workflows after the main R-loop.
