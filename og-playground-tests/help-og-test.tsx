...
// Test with different help articles and categories

export default function HelpOG() {
  // TEST PARAMETERS - Modify these to test different scenarios
  const title = "Hvordan beregne avkastningskrav for næringseiendom?"
  const summary = "En grundig guide til å forstå og beregne avkastningskrav. Lær om risikofri rente, risikopremie og hvordan markedet påvirker kravene."
  const category = "valuation" // Options: overview, getting-started, terms, analysis, valuation

  // Category color mapping for help articles
  const categoryColors: Record<string, { accent: string; label: string }> = {
    overview: { accent: "#cbeef2", label: "Oversikt" },
    "getting-started": { accent: "#a8dadc", label: "Kom i gang" },
    terms: { accent: "#f1faee", label: "Begreper" },
    analysis: { accent: "#8ecae6", label: "Analyse" },
    valuation: { accent: "#e9c46a", label: "Verdivurdering" },
  }
